...
SELECT UPPER(first_name) AS FirstName, UPPER(last_name) AS LastName, hire_date AS HireDate 
FROM employees
ORDER BY HireDate;
...
WHERE birth_date >= '01-01-1950' AND birth_date < '01-01-1960';
...
SELECT order_id, COUNT(order_id) AS number_of_orders FROM order_details
GROUP BY order_id
...
SELECT suppliers.supplier_id, product_name, company_name 
FROM products
...
SELECT first_name || ' ' || last_name || ' can be reached at x' || extension
AS contact_info
FROM employees;
...
/*32. Create a report that shows the average UnitPrice rounded to the next whole number, total number of UnitsInStock and 
...
SELECT o.order_id, c.contact_name, od.unit_price, od.quantity, od.discount
...
SELECT c.customer_id, c.company_name, c.contact_name, c.contact_title, c.address, c.city, c.country, c.phone,
...
SELECT p.product_id, s.company_name, p.product_name,
...
SELECT LEFT(category_name, 5) AS short_info
FROM categories;
...
TRIM(REPLACE(LOWER(company_name), ' ', '') || '@gmail.com') 
...
SELECT p.category_id, s.company_name, p.product_name 
FROM suppliers s
INNER JOIN products p
ON s.supplier_id = p.supplier_id
...
SELECT last_name, first_name, title, EXTRACT(YEAR FROM AGE(birth_date)) 
FROM employees;
...
SELECT product_name || ' weights/is ' || quantity_per_unit || ' and costs $' || unit_price
FROM products;